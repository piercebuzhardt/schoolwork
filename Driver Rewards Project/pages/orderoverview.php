...
<main>
    ...
    ?>
    <a href="../pages/inprogressorderoverview.php">In progress orders</a><br>
    <!-- Search bar -->
    <form name='search' method='POST' action='../pages/orderoverview.php'>
        <table>
            <tr>
	...
	$sql .= " ORDER BY creation_time DESC";
        ...
        if (($nrows = mysqli_num_rows($result)) != 0) {
            echo "<tr><td><b>Username</b></td><td><b>Sponsor Org</b></td><td><b>Authorization Account</b></td><td><b>Creation Time</b></td><td><b>Fulfill Time</b></td><td><b>Shipping address</b></td><td><b>View Details</b></td></tr>";
            for ($i = 0; $i < $nrows; $i += 1) {
		  ...
		  <td>$row[creation_time]</td><td>";
		if($row['fulfill_time'] != NULL) echo "$row[fulfill_time]";
		else echo "To be delivered";
		echo "</td>
		  <td>$row[shipping_address]</td><td><a href='../pages/order_details.php?order_id=".$row['order_id']."'>View</a></td></tr>";
                    }
                } else {
                    echo "<tr><td><a href='../pages/viewaccount.php?viewid=" . $row['driver_account_id'] . "'>$row[username]</a></td><td>$row[org_name]</td><td><a href='../pages/viewaccount.php?viewid=" . $row['auth_account_id'] . "'>$autho[username]</a></td>
		<td>$row[creation_time]</td><td>";
