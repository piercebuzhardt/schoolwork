  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/Gourmet Race - Kirby Super Star.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load Gourmet Race - Kirby Super Star.mp3")+Mix_GetError();

  startMusic();

  sounds.push_back( Mix_LoadWAV("sound/puff.wav") );
  sounds.push_back( Mix_LoadWAV("sound/thud.wav") );
  sounds.push_back( Mix_LoadWAV("sound/float.wav") );
  sounds.push_back( Mix_LoadWAV("sound/fail.wav") );
  sounds.push_back( Mix_LoadWAV("sound/gameover.wav") );
...
void SDLSound::operator[](int index) {
  ...
  currentSound = index;
  Mix_VolumeChunk(sounds[index], volume * .7);
  channels[index] = 
