...
//mine
  std::map<std::string, SDL_Surface*>::const_iterator freePtr = surfaces.begin();
  while(freePtr != surfaces.end()){
	  SDL_FreeSurface(freePtr->second);
	  ++freePtr;
  }
...
//mine
  std::map<std::string, SDL_Texture*>::const_iterator destPtr = textures.begin();
  while(destPtr != textures.end()){
	  SDL_DestroyTexture(destPtr->second);
	  ++destPtr;
  }
...
//mine
  ...
  while(imagePtr != images.end()){
	  ...
	  imagePtr = images.erase(imagePtr);
  }
